...
class Report(object):
    ...
    def addSonarTestInfo(self):
        output = open("testinfo.json",'r')
        mtjsondata = json.load(output)
        output.close()
        mtjd = self.byteify(mtjsondata)
        print mtjd
        sonarlist = ["6","7","8","9","10"]
        resultofsonar = "success"
        for jd in sonarlist:
            if mtjd[jd] != "success" :
                resultofsonar = "failed"

        self.tStatistics.write("<div><br/></div><h2> Map Test Statistics</h2>\n")
        self.tStatistics.write("<table class=\"general\" align=\"center\" cellpadding=\"5\" cellspacing=\"2\" width=\"70%\">\n")
        self.tStatistics.write("<tr><th>test</th>><th>begintime</th><th>endtime</th><th>result</th></tr>\n")
        self.tStatistics.write("<tr><td>Sonar test</td><td>"+ mtjd["begintime"] +"</td><td>" + mtjd["endtime"] + "</td><td>"+ resultofsonar +"</td></tr>\n")
        self.tStatistics.write("</table>\n")

        self.tStatistics.write("<table class=\"general\" align=\"center\" cellpadding=\"5\" cellspacing=\"2\" width=\"70%\">\n")
        self.tStatistics.write("<tr><th>sonarid</th>><th>result</th></tr>\n")
        for jd in sonarlist:
            self.tStatistics.write("<tr><td>" + jd + " </td><td>"+ mtjd[jd] +"</td></tr>\n")
        self.tStatistics.write("</table>\n")
    ...
    def runCreateSonarTestReport(self,ip,productname):
        self.createCSS()
        self.createReport(productname)
        self.getDeviceInfo(ip)
        self.addDeviceInfo()
        self.addSonarTestInfo()
        self.endReport()
